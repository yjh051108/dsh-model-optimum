...
import { pickAnchors, worthAnchoring, residual } from './anchor-core.js';
...
function textOf(result, depth = 0) {
    ...
    if (Array.isArray(r['content'])) {
        const parts = r['content'].map((c) => textOf(c, depth + 1)).filter(Boolean);
        if (parts.length)
            return parts.join('\n');
    }
 ...
 */
function backfill(s, session, persist) {
    if (s.backfilled)
        return;
    s.backfilled = true;
    const diag = {
    ...
    try {
        evs = session?.events;
        diag['eventsType'] = Array.isArray(evs) ? 'array' : typeof evs;
    }
    catch (e) {
        diag['eventsErr'] = String(e);
    }
    if (!Array.isArray(evs)) {
        try {
            evs = session?.log;
            diag['logType'] = Array.isArray(evs) ? 'array' : typeof evs;
        }
        catch (e) {
            diag['logErr'] = String(e);
        }
    }
    if (!Array.isArray(evs) && typeof session?.snapshotEvents === 'function') {
        try {
            evs = session.snapshotEvents();
            diag['snapType'] = Array.isArray(evs) ? 'array' : typeof evs;
        }
        catch (e) {
            diag['snapErr'] = String(e);
        }
    }
    if (Array.isArray(evs)) {
        diag['n'] = evs.length;
        const sample = evs.find((e) => e?.type === 'user/message');
        diag['sampleKeys'] = sample ? Object.keys(sample).slice(0, 12) : null;
        diag['sampleSrc'] = sample ? JSON.stringify(sample?.data?.source ?? null).slice(0, 120) : null;
        for (const e of evs) {
            if (e?.type !== 'user/message')
                continue;
            const src = e?.data?.source;
            if (src?.kind !== 'user' || src?.plugin || e?.data?.goalId)
                continue;
            const txt = plainText(e?.data?.content).trim();
            if (txt)
                s.humans.push({ seq: Number(e?.seq ?? -1), text: txt });
        }
        if (s.humans.length > 200)
            s.humans = s.humans.slice(-200);
    }
...
export function apply(ctx, config) {
    ...
    const stateFile = (sid) => join(ledgerDir(), `${sid}.state.json`);
    const saveState = (sid, s) => {
        try {
            mkdirSync(ledgerDir(), { recursive: true });
    ...
    const hydrate = (sid, s) => {
        try {
            ...
            s.firedKeys = new Set(j['firedKeys'] ?? []);
            s.restatedSeqs = new Set((j['restatedSeqs'] ?? []).map(Number));
    ...
    // ── ① 观测 A：读数指纹（M1）──────────────────────────────────────────────
    ctx.effect(() => ctx.on('tools/result', (...args) => {
        try {
            if (!config.enabled)
                return;
            const exec = args[0];
    ...
    /** 记一次压缩，备好将被埋掉的开发者原话 */
    function queueAnchor(s, sid, shadowed, preCtx, seqs) {
        if (!config.anchorAfterCompaction)
            return;
        s.compactions++;
    ...
    ctx.effect(() => ctx.on('agent/pre-step', async (payload, next) => {
        ...
        try {
            if (!config.enabled)
                return decision;
            const d = decision;
            if (!d || d.kind === 'reject' || !Array.isArray(d.messages))
                return decision;
            const sid = payload?.agent?.session?.id;
            if (!sid)
                return decision;
            const s = st(sid);
            backfill(s, payload?.agent?.session, (rec) => persist(sid, rec));
            // ── M2：压缩后锚点重述（优先，且不受 M1 预算/静默影响）──
            if (s.anchor) {
                const job = s.anchor;
                s.anchor = null;
                for (const m of job.msgs)
                    s.restatedSeqs.add(m.seq);
            ...
            // ── M1：尺子闸 ──
            if (s.fired >= config.maxPerSession)
                return decision;
            if (s.ignoredStreak >= config.silenceAfterIgnored)
                return decision;
            if (Date.now() - s.lastFireAt < config.cooldownMs)
                return decision;
            const hit = detect(s, config);
            if (!hit || s.firedKeys.has(hit.key))
                return decision;
            s.fired++;
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        async execute(args, execCtx) {
                ...
                if (existsSync(f)) {
                    for (const line of readFileSync(f, 'utf8').split('\n')) {
                        ...
                        try {
                            ...
                            if (isIntervene)
                                byAt.set(Number(r['at']), r);
                            if (isVerdict) {
                                const m = byAt.get(Number(r['fireAt']));
                                if (m)
                                    m['verdict'] = r['verdict'];
                            }
                            if (r['rec'] === 'compaction' || r['rec'] === 'anchor-restate')
                                events.push(r);
                        }
                        catch { /* skip */ }
                    }
            ...
            if (live)
                for (const e of live.ledger)
                    byAt.set(Number(e['at']), { ...(byAt.get(Number(e['at'])) ?? {}), ...e });
    ...
    ctx.effect(() => ctx.tools.register(defineTool({
        ...
        async execute(args, execCtx) {
            ...
            if (!s)
                return '模型适配层：本会话尚未初始化（或已重载）。';
            if (s.humans.length === 0)
                return '模型适配层：还没收集到开发者原话。';
...
/** M1 检测：返回命中信号或 null */
function detect(s, config) {
    const r = s.recent;
    if (r.length < config.minRepeat)
        return null;
    const tail = r.slice(-config.minRepeat);
    if (new Set(tail.map((x) => x.resFp)).size !== 1)
        return null; // 结果必须完全一致
